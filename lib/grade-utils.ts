...
export function calculateLetterGrade(
  ...
  for (const grade of gradeScale.scale) {
    if (
      typeof grade.minpercentage === "number" &&
      typeof grade.maxpercentage === "number" &&
      percentage >= grade.minpercentage &&
      percentage <= grade.maxpercentage
    ) {
      return { letter: grade.letter, gpa: grade.gpapoints ?? grade.gpapoints };
    }
  }
  ...
  return {
    ...
    gpa: lowestGrade.gpapoints ?? lowestGrade.gpapoints,
  };
}
...
export function calculateSemesterGPA(
  ...
  const semesterGrades = courseGrades.filter(
    (cg) => cg.semester === semester && cg.year === year
  );
