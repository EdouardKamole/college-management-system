...
export function UserRegistration() {
  ...
  const checkEligibility = () => {
    ...
    for (const courseId of studentData.courseid) {
      ...
      if (
        !course.requirededucationlevel?.includes(
          studentData.educationlevel as any
        )
      ) {
        eligible = false;
        reasons.push(
          `Course "${
            course.name
          }": requires ${course.requirededucationlevel?.join(
            " or "
          )} level education`
        );
      }
      ...
      const hasArts = studentData.subjectcombination.some((subject) =>
        artsSubjects.includes(subject)
      );
      if (hasArts && course.category !== "technical") {
        eligible = false;
        reasons.push(
          `Course "${course.name}": students with arts combinations are not eligible for science-based courses`
        );
      }

      // Check Physics or Math requirement
      const hasPhysicsOrMath =
        studentData.subjectcombination.includes("Physics") ||
        studentData.subjectcombination.includes("Mathematics");
      if (!hasPhysicsOrMath) {
        eligible = false;
        reasons.push(
          `Course "${course.name}": combination must include at least Physics or Mathematics`
        );
      }
      ...
      if (studentData.totalpoints < (course.minimumpoints || 7)) {
        eligible = false;
        reasons.push(
          `Course "${course.name}": minimum ${
            course.minimumpoints || 7
          } points required (you have ${studentData.totalpoints})`
        );
      }
    }
    ...
    if (eligible && studentData.courseid.length > 0) {
      reasons.push(
        "All requirements met - eligible for enrollment in all selected courses"
      );
    }
    ...
    try {
      ...
      const updatedCourses = data.courses.map((course) =>
        studentData.courseid.includes(course.id)
          ? {
              ...course,
              studentids: [...(course.studentids || []), newStudent.id],
            }
          : course
      );
      ...
      for (const courseId of studentData.courseid) {
        ...
        if (courseToUpdate) {
          const { error: courseError } = await supabase
            .from("courses")
            .update({ studentids: courseToUpdate.studentids })
            .eq("id", courseToUpdate.id);
          if (courseError) {
  ...
  const handleStaffSubmit = async () => {
    ...
    try {
      ...
      alert(
        `${
          staffData.role === "admin" ? "Administrator" : "Instructor"
    ...
    >
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Register new students, instructors, or administrators
          </DialogDescription>
        </DialogHeader>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setStudentData((prev) => ({
                          ...prev,
                          totalpoints: Number.isNaN(Number(e.target.value))
                            ? 0
                    ...
                    <small className="text-muted-foreground">
                      Enter the student's total academic points (0-18).
                    </small>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 max-h-32 overflow-y-auto border rounded p-3">
                    {ACCEPTABLE_SUBJECTS.map((subject) => {
                      const isChecked =
                        studentData.subjectcombination.includes(subject);
                      const disableCheckbox =
                        !isChecked &&
                        studentData.subjectcombination.length >= 3;
                      return (
                        <div
                          key={subject}
                          className="flex items-center space-x-2"
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="instructor-qualification">
                      Qualification
                    </Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="instructor-experience">
                      Experience (Years)
                    </Label>
                    <Input
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => generateCredentials("staff")}
                  >
                    Generate
                  </Button>
                </div>
        ...
        <DialogFooter>
          ...
          {activeTab === "student" ? (
            ...
            <Button type="button" onClick={handleStaffSubmit}>
              Register{" "}
              {activeTab === "instructor" ? "Instructor" : "Administrator"}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
